...
# 
# __回答:__不合理。在图像可能偏离在角落，也可能与其它动物图像混合在同一图像中，或有其它几何变化时候等情况还需要使用图像增强功能。我们可以用CNN来检测人脸。
...
# 
# __回答:__ 采用了一个与上面相似的架构。利用了3个卷积层和最大池化层。这些层将帮助模型抽取复杂的特性并去立即不同狗狗品种之间的区别。然后，加入了一个层去flatten output，将这个加入到一个具有64 neurons的全连接层，最后添加dropout避免过拟合。输出采用softmax，将输出分类为133个类别。
...
### TODO: 定义你的网络架构
model.add(Conv2D(16, 5,5 ,input_shape=(224, 224, 3), subsample=(2,2), border_mode='same', name='conv1', activation = 'relu'))
model.add(MaxPooling2D(pool_size=(2,2),strides=(1,1), name='pool_1'))
model.add(Conv2D(32, 5,5 ,border_mode='same', subsample=(2,2), name='conv_2', activation = 'relu'))
model.add(MaxPooling2D(pool_size=(2,2), name='pool_2'))
model.add(Conv2D(64, 3,3,border_mode='same', name='conv_3', activation = 'relu'))
...
# __回答:__ 
# 
# 迁移学习通过预先训练成熟的网络，例如我们采用的IMAGENET等等各种数据集，形成bottleneck features。新的迁移学习方法是把原来已经训练好的训练集拿过来，冻结截取前面的已有卷积层，后面加上全新的未训练层，对于这个新的训练集的训练会由于前面的层已经训练过了，而相对大幅度减少了训练计算成本， 由于预先训练的模型的通用特征，当模型被加入到新的训练层时，可以继续学习，所以，迁移学习有很强的泛化能力。 由于预先训练的模型的参数已经成熟，当模型加入新的训练层的时候，可训练的参数可以大幅度减少，fine-tune的训练更稳定，计算时间更少，训练性能更高。 对比CNN模型架构的演进，VGG主要目的是增加网络深度，减少卷积核的大小。google的inception增加了卷积模块的功能，inceptionV2新增加了batch normalization，进一步加快了训练速度，Resnet是microsoft中国提出的，是目前的主流算法，解决了深度网络中的退化问题。总体来说，采用合理的CNN网络架构，再通过迁移学习的特征提取与fine-tunning，可以对新的数据集有很好的泛化能力，能够识别成熟数据集以外的更多新的数据目标。
# 
# ***
# 本模块功能：
...
model.add(Dropout(0.5))
Resnet50_model.add(Dense(860, activation='tanh'))
model.add(Dropout(0.5))
...
### TODO: 训练模型
Resnet50_modelCheckpoint = ModelCheckpoint(filepath='saved_models/weights.best.Resnet50TGX.hdf5',verbose=1,save_best_only=True)
Resnet50_model.fit(train_Resnet50,train_targets,validation_data=(valid_Resnet50,valid_targets),
                  epochs=30,batch_size=20,callbacks=[Resnet50_modelCheckpoint],verbose=1)
...
Resnet50_model.load_weights('saved_models/weights.best.Resnet50TGX.hdf5')
...
### TODO: 在测试集上计算分类准确率
predictions_Resnet50 = [np.argmax(Resnet50_model.predict(np.expand_dims(feature, axis=0))) for feature in test_Resnet50]
...
### 然后返回此模型所预测的狗的品种
def ResNet_predict_dog_class(img_path):
    # 提取bottleneck特征
    bottleneck_feature_ResNet50 = extract_Resnet50(path_to_tensor(img_path))
    # 获取预测向量
    predicted_vector = Resnet50_model.predict(bottleneck_feature_ResNet50)
    # 返回此模型预测的狗的品种
    return dog_names[np.argmax(predicted_vector)]
...
def breeds_detector(img_path):
    if dog_detector(img_path):
        print("dog breed is..{}\n\n".format(ResNet_predict_dog_class(img_path)))
        plot_image(img_path)
